...
void evaluation_mnbrak(optimizedVectorFunction* func,VectorXd& axv,VectorXd& bxv,VectorXd& cxv,double* fa,double* fb,double* fc){
	...
	stdVct.normalize();
	VectorXd origin=bxv;//origin���ꎟ������̉��̌��_�Ƃ�stdVct orientation�𐳂̕����Ƃ���
	VectorXd input;
	double ax[1];
	double bx[1];
	double cx[1];
...
double brent_method(optimizedVectorFunction* func,VectorXd& ax,VectorXd& bx,VectorXd& cx,float tol,VectorXd& xmin){
	...
	VectorXd origin=bx;//origin���ꎟ������̉��̌��_�Ƃ�stdVct orientation�𐳂̕����Ƃ���
	VectorXd input;
	stdVct.normalize();
...
//	cout<<iter<<","<<ans<<","<<p(0)<<","<<p(1)<<","<<p(2)<<","<<p(3)<<","<<p(4)<<","<<p(5)<<endl;
	for(int iter=0;;iter++){
		...
		if(2.0*fabs(fp-ans)<=dtol*(fabs(fp)+fabs(ans))){//�I������
			return;
		}
